...
var Reveal = (function(){
	...
	function onDocumentKeyDown( event ) {
		...
		// Костыль, следящий за положением экрана
		// if ( ! isSlideActive ) return true;
		...
		if( !touch.handled ) {
			...
			// There was only one touch point, look for a swipe
				...
				if( deltaX > touch.threshold && Math.abs( deltaX ) > Math.abs( deltaY ) ) {
					...
					// Костыль для тач-устройств - если нет слайда сверху, делаем прокрутку страницы
					touch.handled = true;
					if(availableRoutes().up) {
						navigateUp();
					} else return true;
					...
					// Аналог. костыль для движения вниз
					touch.handled = true;
